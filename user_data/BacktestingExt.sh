...
function_optimizeBot() {
    ...
    #docker-compose run --rm freqtrade backtesting -c ${CONFIG} --strategy ${STRATEGY} --timerange=${START_TIMERANGE}-${END_TIMERANGE}  --timeframe-detail 15m --enable-protections --breakdown month day --export trades
    docker-compose run --rm freqtrade backtesting -c ${CONFIG} --strategy ${STRATEGY} --timerange=${START_TIMERANGE}-${END_TIMERANGE} --breakdown month day --export trades

    function_printOutput "Plotting profits for timerange between ${startDatetime} and ${endDatetime}"
    #docker-compose run --rm freqtrade plot-dataframe -c ${CONFIG} --strategy ${STRATEGY} --timerange=${START_TIMERANGE}-${END_TIMERANGE} -p AMP/USDT
    #docker-compose run --rm freqtrade plot-profit -c ${CONFIG} --strategy ${STRATEGY} --timerange=${START_TIMERANGE}-${END_TIMERANGE} --timeframe 1d #${TIMEFRAME}
...
function_displayHelp() {
    echo "e.g. -s BBMeanReversion 15m 315 2021-01-05 22:00:00 2021-01-06 08:30:00"
    echo "-s  Strategy Class"
